...
export const createDeckBundle = (deck: DeckTemplate, assets: AssetTemplate[]): DeckBundle => {
  const deckAssetIds = new Set(getDeckAssetIds(deck));
  return {
    version: 1,
    kind: "deck",
    exportedAt: Date.now(),
    deckTemplate: deck,
    assets: assets.filter((asset) => deckAssetIds.has(asset.id))
  };
};
...
export const parseDeckBundle = (input: string): DeckBundle => {
  ...
  if (value.version !== 1 || value.kind !== "deck" || !value.deckTemplate || !Array.isArray(value.assets)) {
    throw new Error("Imported file is not a valid deck export.");
  }
